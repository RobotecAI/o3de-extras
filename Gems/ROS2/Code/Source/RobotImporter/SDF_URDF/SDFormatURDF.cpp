...
namespace sdformat_urdf
{
    ...
    urdf::LinkSharedPtr convert_link(const sdf::Link& sdf_link, const std::string& joint_frame, sdf::Errors& errors);

    /// \brief Convert SDFormat Joint to URDF Joint
    urdf::JointSharedPtr convert_joint(const sdf::Joint& sdf_joint, sdf::Errors& errors);

    urdf::Pose convert_pose(const ignition::math::Pose3d& sdf_pose);

    urdf::GeometrySharedPtr convert_geometry(const sdf::Geometry& sdf_geometry, sdf::Errors& errors);
} // namespace sdformat_urdf

urdf::ModelInterfaceSharedPtr sdformat_urdf::parse(const std::string& data, sdf::Errors& errors)
{
...
urdf::ModelInterfaceSharedPtr sdformat_urdf::sdf_to_urdf(const sdf::Root& sdf_dom, sdf::Errors& errors)
{
    if (sdf_dom.WorldCount() > 0u)
    {
        errors.emplace_back(sdf::ErrorCode::STRING_READ, "SDFormat xml has a world; but only a single model is supported");
        return nullptr;
    }
    if (nullptr == sdf_dom.Model())
    {
        errors.emplace_back(sdf::ErrorCode::ELEMENT_MISSING, "SDFormat xml has no models; need at least one");
        return nullptr;
    }
    return convert_model(*sdf_dom.Model(), errors);
}

urdf::ModelInterfaceSharedPtr sdformat_urdf::convert_model(const sdf::Model& sdf_model, sdf::Errors& errors)
{
    ...
    {
        errors.emplace_back(sdf::ErrorCode::STRING_READ, "<model> tags with <pose> are not currently supported by sdformat_urdf");
        return nullptr;
    }
    ...
    for (uint64_t l = 0; l < sdf_model.LinkCount(); ++l)
    {
        ...
        if (!sdf_link)
        {
            errors.emplace_back(sdf::ErrorCode::STRING_READ, "Failed to get sdf link");
            return nullptr;
        }
        ...
        auto pair = urdf_model->links_.emplace(sdf_link->Name(), convert_link(*sdf_link, relative_joint_name, errors));

        if (!pair.second)
        {
            errors.emplace_back(sdf::ErrorCode::STRING_READ, "Failed to create element in links map");
            return nullptr;
        }
        ...
        if (!urdf_link)
        {
            errors.emplace_back(sdf::ErrorCode::STRING_READ, "Failed to convert sdf::link [" + sdf_link->Name() + "] to urdf::Link");
            return nullptr;
        }
    }
    ...
    {
        errors.emplace_back(sdf::ErrorCode::STRING_READ, "Failed to get sdf canonical link");
        return nullptr;
    }
    auto iter = urdf_model->links_.find(sdf_canonical_link->Name());
    if (iter == urdf_model->links_.end())
    {
        errors.emplace_back(sdf::ErrorCode::STRING_READ, "Failed to find sdf canonical link [" + sdf_canonical_link->Name() + "]");
        return nullptr;
    }
    ...
    for (uint64_t j = 0; j < sdf_model.JointCount(); ++j)
    {
        ...
        if (!sdf_joint)
        {
            errors.emplace_back(sdf::ErrorCode::STRING_READ, "Failed to get sdf joint");
            return nullptr;
        }
        ...
        if (!pair.second)
        {
            errors.emplace_back(sdf::ErrorCode::STRING_READ, "Failed to create element in joints map");
            return nullptr;
        }
        ...
        if (!urdf_joint)
        {
            errors.emplace_back(sdf::ErrorCode::STRING_READ, "Failed to convert sdf::joint [" + sdf_joint->Name() + "] to urdf::Joint");
            return nullptr;
        }
    }
        ...
        while (joint_iter != joints_to_visit.end())
        {
            ...
            if (sdf_joint->ParentLinkName() == sdf_parent_link->Name())
            {
                ...
                if (!pose_errors.empty())
                {
                    ...
                    errors.emplace_back(
                        ...
                        "Failed to get transform from joint [" + sdf_joint->Name() + "] to link [" + sdf_parent_link->Name() + "]");
                    return nullptr;
                }
                ...
                if (!sdf_child_link)
                {
                    errors.emplace_back(sdf::ErrorCode::STRING_READ, "Failed to get child link [" + child_link_name + "]");
                    return nullptr;
                }
                ...
                {
                    errors.emplace_back(sdf::ErrorCode::STRING_READ, "Found kinematic loop at joint [" + sdf_joint->Name() + "]");
                    return nullptr;
                }
            ...
            else if (sdf_joint->ChildLinkName() == sdf_parent_link->Name())
            {
                ...
                if (sdf_parent_link == sdf_canonical_link)
                {
                    ...
                    errors.emplace_back(
                        sdf::ErrorCode::STRING_READ, "Canonical link must not be a child of a joint [" + sdf_parent_link->Name() + "]");
                }
                else
                {
                    // This link must be a child of two joints - kinematic loop :(
                    errors.emplace_back(
                        sdf::ErrorCode::STRING_READ, "Link [" + sdf_parent_link->Name() + "] must only be a child of one joint");
                }
                return nullptr;
            }
            else
    ...
    if (visited_links.size() < sdf_model.LinkCount())
    {
        ...
        errors.emplace_back(sdf::ErrorCode::STRING_READ, "Found multiple root links - must only have one link not a child of any joint");
        return nullptr;
    }
    else if (visited_links.size() > sdf_model.LinkCount())
    {
        errors.emplace_back(sdf::ErrorCode::STRING_READ, "Algorithm error - visited more links than exist, please file a bug report");
        return nullptr;
    }
    if (!joints_to_visit.empty())
    {
        errors.emplace_back(sdf::ErrorCode::STRING_READ, "Algorithm error - did not visit all joints, please file a bug report");
        return nullptr;
    }
...
urdf::LinkSharedPtr sdformat_urdf::convert_link(const sdf::Link& sdf_link, const std::string& relative_joint_name, sdf::Errors& errors)
{
    ...
    if (!relative_joint_name.empty())
    {
        ...
        {
            errors.insert(errors.end(), pose_errors.begin(), pose_errors.end());
            errors.emplace_back(sdf::ErrorCode::STRING_READ, "Failed to get transform of link [" + sdf_link.Name() + "]");
            return nullptr;
        }
    }
    ...
    for (uint64_t vi = 0; vi < sdf_link.VisualCount(); ++vi)
    {
        ...
        if (!sdf_visual)
        {
            errors.emplace_back(sdf::ErrorCode::STRING_READ, "Failed to get visual on link [" + sdf_link.Name() + "]");
            return nullptr;
        }
        ...
        if (!pose_errors.empty())
        {
            ...
            errors.emplace_back(
                sdf::ErrorCode::STRING_READ,
                "Failed to get transform from visual [" + sdf_visual->Name() + "] to link [" + sdf_link.Name() + "]");
            return nullptr;
        }
        ...
        {
            errors.emplace_back(sdf::ErrorCode::STRING_READ, "Failed to convert geometry on visual [" + sdf_visual->Name() + "]");
            return nullptr;
        }
        ...
        if (sdf_material)
        {
            ...
            urdf_material->color.r = 0.4 * sdf_material->Ambient().R() + 0.8 * sdf_material->Diffuse().R();
            urdf_material->color.g = 0.4 * sdf_material->Ambient().G() + 0.8 * sdf_material->Diffuse().G();
            urdf_material->color.b = 0.4 * sdf_material->Ambient().B() + 0.8 * sdf_material->Diffuse().B();
            urdf_material->color.a = 0.4 * sdf_material->Ambient().A() + 0.8 * sdf_material->Diffuse().A();
            ...
            if (!sdf_material->Lighting())
            {
                RCUTILS_LOG_WARN_NAMED(
                    "sdformat_urdf",
                    "SDFormat visual [%s] has <material><lighting>,"
                    " but URDF does not support this",
                    sdf_visual->Name().c_str());
            }
    ...
    for (uint64_t vi = 0; vi < sdf_link.CollisionCount(); ++vi)
    {
        ...
        if (!sdf_collision)
        {
            errors.emplace_back(sdf::ErrorCode::STRING_READ, "Failed to get collision on link [" + sdf_link.Name() + "]");
            return nullptr;
        }
        ...
        if (!pose_errors.empty())
        {
            ...
            errors.emplace_back(
                sdf::ErrorCode::STRING_READ,
                "Failed to get transform from collision [" + sdf_collision->Name() + "] to link [" + sdf_link.Name() + "]");
            return nullptr;
        }
        ...
        {
            errors.emplace_back(sdf::ErrorCode::STRING_READ, "Failed to convert geometry on collision [" + sdf_collision->Name() + "]");
            return nullptr;
        }
...
urdf::JointSharedPtr sdformat_urdf::convert_joint(const sdf::Joint& sdf_joint, sdf::Errors& errors)
{
    ...
    default:
        errors.emplace_back(sdf::ErrorCode::STRING_READ, "Unsupported joint type on joint [" + sdf_joint.Name() + "]");
        return nullptr;
    }
    ...
    if ((urdf::Joint::FIXED != urdf_joint->type) && (urdf::Joint::FLOATING != urdf_joint->type))
    {
        ...
        if (nullptr == sdf_axis)
        {
            errors.emplace_back(sdf::ErrorCode::STRING_READ, "Axes missing for joint [" + sdf_joint.Name() + "]");
            return nullptr;
        }
        ...
        if (!axis_errors.empty())
        {
            ...
            errors.emplace_back(
                sdf::ErrorCode::STRING_READ,
                "Failed to get transform of joint axis in frame [" + sdf_axis->XyzExpressedIn() + "] to joint [" + sdf_joint.Name() + "]");
            return nullptr;
        }
        ...
        if (0.0 != sdf_axis->SpringReference())
        {
            RCUTILS_LOG_WARN_NAMED(
                "sdformat_urdf",
                "SDFormat Joint [%s] given non-default value for <spring_reference>,"
                " but URDF does not support this",
                sdf_joint.Name().c_str());
        }
        if (0.0 != sdf_axis->SpringStiffness())
        {
            RCUTILS_LOG_WARN_NAMED(
                "sdformat_urdf",
                "SDFormat Joint [%s] given non-default value for <spring_stiffness>,"
                " but URDF does not support this",
                sdf_joint.Name().c_str());
        }
        ...
        if (urdf::Joint::CONTINUOUS != urdf_joint->type)
        {
            ...
            if (1.0 != sdf_axis->Dissipation())
            {
                RCUTILS_LOG_WARN_NAMED(
                    "sdformat_urdf",
                    "SDFormat Joint [%s] given non-default value for <dissipation>,"
                    " but URDF does not support this",
                    sdf_joint.Name().c_str());
            }
            if (1e8 != sdf_axis->Stiffness())
            {
                RCUTILS_LOG_WARN_NAMED(
                    "sdformat_urdf",
                    "SDFormat Joint [%s] given non-default value for <stiffness>,"
                    " but URDF does not support this",
                    sdf_joint.Name().c_str());
            }
        }
...
urdf::Pose sdformat_urdf::convert_pose(const ignition::math::Pose3d& sdf_pose)
{
    urdf::Pose pose;
...
urdf::GeometrySharedPtr sdformat_urdf::convert_geometry(const sdf::Geometry& sdf_geometry, sdf::Errors& errors)
{
    ...
    {
        errors.emplace_back(sdf::ErrorCode::STRING_READ, "Plane geometry cannot be converted to urdf C++ structures");
        return nullptr;
    }

    errors.emplace_back(sdf::ErrorCode::STRING_READ, "Unknown geometry shape");
    return nullptr;
}
